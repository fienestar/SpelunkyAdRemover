...
const jar = CookieJar.deserializeSync(JSON.parse(process.env.COOKIE_JAR as string));
...
async function processPost(post: PostInfo)
{
    ...
    if(hasAV && title != post.title){
        const result = await dc.deletePost({ galleryId: targetGalleryId, postId: post.id, jar })
        if(result.success){
...
dc.getPostList({ page: 1, galleryId: targetGalleryId, boardType: 'all', jar })
